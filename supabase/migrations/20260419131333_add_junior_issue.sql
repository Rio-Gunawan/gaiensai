alter table "public"."ticket_issue_controls" add column "junior_class_mode" text not null default 'open'::text;

alter table "public"."ticket_issue_controls" add column "junior_entry_only_mode" text not null default 'open'::text;

alter table "public"."ticket_issue_controls" add column "junior_gym_mode" text not null default 'open'::text;

alter table "public"."ticket_issue_controls" add constraint "ticket_issue_controls_junior_class_mode_check" CHECK ((junior_class_mode = ANY (ARRAY['open'::text, 'only-own'::text, 'public-rehearsals'::text, 'auto'::text, 'off'::text]))) not valid;

alter table "public"."ticket_issue_controls" validate constraint "ticket_issue_controls_junior_class_mode_check";

alter table "public"."ticket_issue_controls" add constraint "ticket_issue_controls_junior_entry_only_mode_check" CHECK ((junior_entry_only_mode = ANY (ARRAY['open'::text, 'only-own'::text, 'public-rehearsals'::text, 'auto'::text, 'off'::text]))) not valid;

alter table "public"."ticket_issue_controls" validate constraint "ticket_issue_controls_junior_entry_only_mode_check";

alter table "public"."ticket_issue_controls" add constraint "ticket_issue_controls_junior_gym_mode_check" CHECK ((junior_gym_mode = ANY (ARRAY['open'::text, 'only-own'::text, 'public-rehearsals'::text, 'auto'::text, 'off'::text]))) not valid;

alter table "public"."ticket_issue_controls" validate constraint "ticket_issue_controls_junior_gym_mode_check";
...
begin
  ...
  if not v_is_day_ticket then
    ...
    if not found or (v_role <> 'student' and v_role <> 'junior') then
      raise exception 'only students or junior can issue tickets';
    end if;
  end if;
