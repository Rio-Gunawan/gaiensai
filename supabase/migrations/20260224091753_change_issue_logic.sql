alter table "public"."ticket_code_counters" add constraint "ticket_code_counters_last_value_check" CHECK (((last_value >= 0) AND (last_value <= 15))) not valid;

alter table "public"."ticket_code_counters" validate constraint "ticket_code_counters_last_value_check";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.increment_ticket_code_counter(p_prefix text, p_increment integer)
 RETURNS bigint
 LANGUAGE plpgsql
 SECURITY DEFINER
