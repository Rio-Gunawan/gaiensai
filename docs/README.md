...
- オフラインでも使用可能。表示側はスクショOK、読み取り側もデフォルトはネットを使うが落ちても続行可能な設計。
- 複雑な操作が不要。特に、外部の人はリンクをクリックするだけで使えるようになります。
- それなりのセキュリティ。金銭的価値がないチケットへの使用を想定しているので、悪ふざけ程度の不正な複製は防ぎつつ、頑丈なセキュリティよりは使いやすさを優先。
